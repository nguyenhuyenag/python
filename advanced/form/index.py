...
def create():
    if FIRSTNAME.get() == "" or LASTNAME.get() == "" or GENDER.get() == "" or ADDRESS.get() == "" or USERNAME.get() == "":
        txt_result.config(text="Các ô nhập không được để trống!", fg="red")
    else:
        if GENDER.get() == 0:
            gioitinh = "Nữ"
        elif GENDER.get() == 1:
        ...
        sql = "INSERT INTO user (username, firstname, lastname, gender, address) VALUES (%s, %s, %s, %s, %s)"
        val = (USERNAME.get(), FIRSTNAME.get(), LASTNAME.get(), gioitinh, ADDRESS.get())
        cursor.execute(sql, val)
        tree.delete(*tree.get_children())
        ...
        clear()
        cursor.close()
        txt_result.config(text="Thêm tài khoản thành công", fg="green")
...
def delete():
    if not tree.selection():
        ...
        if result == 'yes':
            ...
            datasource.commit()
            txt_result.config(text="Thao tác thành công", fg="green")
...
fields = ["Mã", "Tài khoản", "Họ", "Tên", "Giới tính", "Địa chỉ"]
